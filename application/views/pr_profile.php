...
<head>
    ...
    <link rel="stylesheet" href="<?php echo base_url("css/pr_p/style-cards.css");?>">
    <link rel="stylesheet" href="<?php echo base_url("css/pr_p/style-comments.css");?>">
    <link rel="stylesheet" src="<?php echo base_url("css/pr_p/style-modals.css");?>">
    <link rel="stylesheet" href="<?php echo base_url("css/pr_p/style-private.css");?>">

    <script defer src="<?php echo base_url("js/pr_p/script.js");?>"></script>
    <style>
        .post-container {
  height: 100vh;
  width: 100%;
...
<body>
 <nav class="post-container">
            ...
            <div class="posted-comment" id="pscmts">
                ...
                <div class="dark-container">
                    ...
                    <ul class="comment-react">
                        <li class="react-like">
                            <a href="#">
                                <img src="svg-sources/thumb_up_white_24dp.svg" alt="ico" />
                            </a>
                        </li>

                        <li class="react-dislike">
                            <a href="#">
                                <img src="svg-sources/thumb_down_white_24dp.svg" alt="ico" />
                            </a>
                        </li>
                    </ul>
                </div>
    ...
    <div class="bg-glass container-lg">
        <img class="card-img-top" src="<?php echo $like_result[0]['IMAGES']; ?>" alt="Card image cap" id="im2">
        <div class="card-body little-profile text-center">
            <div class="pro-img">
                <button class="pro-img-svg">
                    <img src="<?php echo base_url("svg-sources/photo_camera_white_24dp.svg");?>" id="im1" alt="">
                </button>
                <img class="pro-img-child" src="<?php echo $like_result[0]['IMAGES']; ?>" alt="user">
                <input type="file" id="imp-ip" accept="image/*" style="display: none;">
            </div>
            ...
            <div class="pfp-bio">
                <?php
                if(isset($like_result[0]['BIO']))
                {
               echo ('<p contenteditable="true" id="biotxt">'.$like_result[0]['BIO'].'</p>');
                }
                else
                {
                    echo ('<p contenteditable="true" id="biotxt">Lorem ipsum dolor sit amet consectetur adipisicing elit. Excepturi enim quisquam totam.</p>');

                }
                ?>
                <button id="biosumbits" onclick="biosumbit()">
                    <img src="<?php echo base_url("svg-sources/edit_white_24dp.svg");?>" alt="">
                </button>
            </div>
            <?php $xnm= "'".base_url("index.php/Regcontroller")."'";?>
            <button onclick="location.href=<?php echo $xnm; ?>;" id="follow" class="follow-btn">
                <?php 
        ...
        $(document).ready(function() {
            ...
            $('#imp-ip').change(function() {
                var form_data = new FormData();
                form_data.append('image', $('#imp-ip')[0].files[0]);
                $.ajax({
                    url: '<?php echo base_url("index.php/ProfileController/imupdate"); ?>',
                    type: 'POST',
                    processData: false,
                    contentType: false,
                    data: form_data,
                    success: function(data) {
                        var image_url = JSON.parse(data).url;
                        $('#im1').attr('src', image_url);
                        $('#im2').attr('src', image_url);
                        setTimeout(() => {
                            location.reload();
                        }, 1000);
                    }
                        ...
                        if (!post_exists(posts[i].ID)) {
                            ...
                            if (posts[i].IMAGE == null) {
                                $('#postslist').append(
                                    ...
                                    posts[i].TITLE +
                                    '</a></h1> <div class="postcard__subtitle small"> <time datetime="2020-05-25 12:00:00"> <i class="fas fa-calendar-alt mr-2"></i>Mon, May 25th 2020 </time> </div> <div class="postcard__bar"></div> <div class="postcard__preview-txt">' +
                                    atob(posts[i].POST) +
                                    ...
                                    '</div> </a> </div> <ul class="postcard__tagbox"> <li class="tag__item" id="dl' +
                                    posts[i].ID + '"> <a href="javascript:pdel(' + posts[i].ID +
                                    ');" ><img src="<?php echo base_url("svg-sources/delete_white_24dp.svg")?>" alt="ico" /></a> <!-- <i class="fas fa-tag mr-2"></i>Upvote --> </li>  <li class="tag__item comment-button play blue"> <a href="javascript:cmtload(' +
                                    posts[i].ID +
                                ...
                                var imx = "https://campuspace.online/uploads/";
                                var bas = "'";
                                $('#postslist').append(
                                    '<article class="postcard dark blue"> <a class="postcard__img_link" href="javascript:mageprev(' +bas + imx + posts[i].IMAGE + bas +')"> <img class="postcard__img" src="' +
                                    imx + posts[i].IMAGE +
        ...
        function biosumbit() {
            ...
            var textp = $('#biotxt').text();
            if (textp.length === 0) {
                toastMixin.fire({
                    title: "&#128552; Bio can't be empty",
                    icon: 'error',
                });
            } else {
                var cdata = {
                ...
                //  console.log(cdata);
                $.ajax({
                    url: '<?php echo base_url("index.php/profilecontroller/bioup") ?>',
                    method: 'POST',
                    data: cdata,
                    success: function(response) {
                    ...
                    if (lenx > 0) {
                        ...
                        for (var lc = 0; lc < lenx; lc++) {
                            $('#pscmts').append(
                                ...
                                response[lc].MASKER_IMAGE + '" alt=""> </a> <span>' + response[lc]
                                .COMMENT +
