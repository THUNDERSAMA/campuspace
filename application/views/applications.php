...
<head>
    ...
    <link rel="stylesheet" href="<?php echo base_url("css/sr/style-profile.css");?>">
    <link rel="stylesheet" href="<?php echo base_url("css/sr/style-cards.css");?>">
</head>
...
$datas = json_decode(json_encode($datas), true); foreach ($datas as $choice) { ?>
                    <div class="col-sm-6 col-md-4 col-lg-4">
                        <div class="card">
                            <img src="<?php echo $choice['IMAGES']; ?>" class="card-img-top mx-auto" alt="image">
                            <div class="card-body">
                            
                              <h5 class="card-title"><?php echo $choice['NAME']; ?></h5>
                              <p class="card-text"><?php echo substr($choice['APPLICATION'], 0, 100); ?></p>
                             
                              <div class="btn-group" role="group" aria-label="Basic example">
                              
  <button type="button" class="btn btn-primary" onclick="accept(<?php echo $choice['application_id']; ?>)">Accept</button>
  <button type="button" onclick="vb(this)" class="btn btn-primary">Read</button>
  <button type="button" class="btn btn-primary" onclick="decline(<?php echo $choice['application_id']; ?>)">Decline</button>
  <P style="display:none;"><?php echo $choice['APPLICATION'] ; ?></p>
</div>
                            </div>
                          </div>
                    </div>
  ...
  var titles = card.querySelector('.card-title').textContent;
  var img = card.querySelector("img.card-img-top").getAttribute("src");
  while (sibling) {
...
$.ajax({
      type: "POST",
      url: "<?php echo base_url('index.php/ProfileController/aacept'); ?>",
      data: cdata,
...
$.ajax({
      type: "POST",
      url: "<?php echo base_url('index.php/ProfileController/adecline'); ?>",
      data: cdata,
      success: function(response) {
        console.log(response);
        if (response=="ok") {
